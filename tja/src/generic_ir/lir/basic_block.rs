use crate::{
    Immediate,
    lir::{
        Function, GlobalValueIdx, Instruction, InstructionId, TargetInstruction, Terminator, Ty,
...
pub(crate) struct Builder<'a, TI: TargetInstruction, I: InstructionInserter<TI>> {
...
impl<'a, TI: TargetInstruction, I: InstructionInserter<TI>> Builder<'a, TI, I> {
    pub(super) fn new(inserter: I, func: &'a mut Function<TI>, decls: &'a Declarations) -> Self {
        Self {
            inserter,
            func,
            decls,
...
impl<TI: TargetInstruction> InstructionInserter<TI> for AppendInstrInserter<'_> {
    fn insert_instr(
        ...
        let results = match &instr {
            ...
            Instruction::Target(instr) => instr.result_tys(ty),
        };
